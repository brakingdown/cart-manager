...
public class MenuController {
    ...
    //欢迎页控制层
    public void welcomeMenuController(String choice) throws  SQLException {

        switch (choice) {
            case "1" -> userLogin();
            case "2"-> userRegister();//注册
            ...
            //错误操作
            default -> System.out.println("您输入的操作有误，请重新选择");
    ...
    private void forgotPassword() throws SQLException {
        ...
        User user = userController.getUserByUsernameAndEmail(username, email);
        if(user!=null){
            String newPwd = pwdClass.getRandomPassword(RANDOM_PWD_LEN);
            userController.modifyUserPwdByID(String.valueOf(user.getId()),newPwd);
    ...
    public void adminMenuController(String choice) throws SQLException {

        switch (choice) {
            case "1" -> modifyAdminPassword();
            case "2" -> resetUserPassword();
            case "3" -> {
    ...
    private void modifyAdminPassword() throws SQLException{
        System.out.println("\n              管理员界面>管理员密码修改            ");
        System.out.println("***********************************************");
    ...
    public void userManagerController(String choice) throws  SQLException {
        switch (choice) {
            case "1" -> showAllUsers();
            case "2" -> deleteUserByPhoneNumb();
            case "3" -> {
                menu.queryUserMenu();
                return;
            }
    ...
    private void deleteUserByPhoneNumb() throws SQLException{
        System.out.println("\n          管理员界面>客户管理>删除客户信息          ");
        System.out.println("***********************************************");
    ...
    public void queryUserController(String choice) throws  SQLException {
        switch (choice) {
            case "1" -> queryUserByID();
            case "2" -> queryUserByName();
            case "3" -> queryAllUser();
            case "4" -> {
                menu.userManagerMenu();
                return;
    ...
    private void queryUserByID() throws SQLException{
        System.out.println("\n      管理员界面>客户管理>查询客户信息>按ID查询       ");
        System.out.println("***********************************************");
        System.out.println("请输入客户ID");
        userController.showUser(userController.getUserById(scanner.next()));
    }
    private void queryUserByName() throws SQLException{
        System.out.println("\n     管理员界面>客户管理>查询客户信息>按用户名查询     ");
        System.out.println("***********************************************");
    ...
    private void queryAllUser() throws SQLException{
        System.out.println("\n    管理员界面>客户管理>查询客户信息>查询所有客户信息   ");
        System.out.println("***********************************************");
        userController.showAllUsers();
    }

    public void goodsController(String choice) throws  SQLException {
        switch (choice) {
            case "1" -> ShowAllGoods();
            case "2" -> addGoods();
            case "3" -> modifyGoods();
    ...
    private void ShowAllGoods() throws SQLException{
        System.out.println("\n        管理员界面>商品管理>列出所有商品信息        ");
        System.out.println("***********************************************");
        goodsController.showAllGoods();
    }
    private void addGoods() throws SQLException{
        System.out.println("\n          管理员界面>商品管理>添加商品信息          ");
        System.out.println("***********************************************");
        goodsController.addGoods();
    }
    private void modifyGoods() throws SQLException {
        System.out.println("\n          管理员界面>商品管理>修改商品信息          ");
        System.out.println("***********************************************");
    ...
    private void deleteGoods() throws SQLException {
        System.out.println("\n          管理员界面>商品管理>删除商品信息          ");
        System.out.println("***********************************************");
        System.out.println("在库商品:");
        System.out.println();
    ...
    public void userMenuController(String choice) throws  SQLException {

        switch (choice) {
            case "1" -> {
                changeUserPassword();//修改密码
                menu.welcomeMenu();//重新登录
                return;
            }
            case "2" -> {
                menu.shoppingMenu();
    ...
    private void changeUserPassword() throws SQLException {
        System.out.println("\n                用户界面>修改密码                ");
        System.out.println("***********************************************");
        ...
        while (true) {
            newPwd = scanner.next();
            if (!pwdClass.checkPassword(newPwd)) {
                System.out.println("密码要求大于8位且包含大小写字母与特殊符号,请重新输入:");
            } else break;
        }
        userController.modifyUserPwdByID(String.valueOf(user.getId()),newPwd);  //更新数据库
        System.out.println("修改成功,请重新登录");
    }

    public void shoppingController(String choice) throws SQLException {
        switch (choice){
            case"1"-> addToShoppingCart();
            case"2"-> removeFromShoppingCart();
            case"3"-> modifyPurchaseNumb();
            case"4"-> checkoutShoppingCart();
            case"5"-> viewShoppingHistory();
            case"6"->{
                menu.userMenu();
                return;
            }
    ...
    private void removeFromShoppingCart() throws SQLException {
        System.out.println("\n            用户界面>购物管理>移除购物车            ");
        System.out.println("***********************************************");
    ...
    private void modifyPurchaseNumb() throws SQLException {
        System.out.println("\n            用户界面>购物管理>修改购买数量            ");
        System.out.println("***********************************************");
        System.out.println("购物车列表:");
        System.out.println();
    ...
    private void checkoutShoppingCart() throws SQLException {
        System.out.println("\n            用户界面>购物管理>购物车结算            ");
        System.out.println("***********************************************");
        shoppingCartController.showAllShoppingCartGoods();
        System.out.println();
        shoppingCartController.pay(user);
    }
    private void viewShoppingHistory() throws SQLException{
        System.out.println("\n            用户界面>购物管理>查看购物历史            ");
        System.out.println("***********************************************");
